...
public class SocialFeedResource {
    ...
    public ResponseEntity<SocialFeed> createSocialFeed(@RequestBody SocialFeed socialFeed) throws URISyntaxException {
    ...
    public ResponseEntity<SocialFeed> updateSocialFeed(
        ...
        @RequestBody SocialFeed socialFeed
    ...
    public List<SocialFeed> getAllSocialFeeds(@RequestParam(required = false) String filter) {
        if ("charityprofile-is-null".equals(filter)) {
            log.debug("REST request to get all SocialFeeds where charityProfile is null");
            return StreamSupport
                .stream(socialFeedRepository.findAll().spliterator(), false)
                .filter(socialFeed -> socialFeed.getCharityProfile() == null)
