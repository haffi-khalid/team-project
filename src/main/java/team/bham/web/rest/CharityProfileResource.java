...
public class CharityProfileResource {
    ...
    public List<CharityProfile> getAllCharityProfiles(@RequestParam(required = false) String filter) {
        if ("charityadmin-is-null".equals(filter)) {
            log.debug("REST request to get all CharityProfiles where charityAdmin is null");
            return StreamSupport
                .stream(charityProfileRepository.findAll().spliterator(), false)
                .filter(charityProfile -> charityProfile.getCharityAdmin() == null)
